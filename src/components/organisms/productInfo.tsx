...
const ProductInfo = ({ description, chars }: ProductDescriptionProps) => {
    ...
    return (
        <section>
            <div className="mb-4 border-b border-gray-200 dark:border-gray-400 dark:bg-slate-800">
            ...
            <div id="myTabContent">
                {description && active === "description" &&
                    <div className="py-4 bg-gray-50 rounded-lg dark:bg-gray-800" id="description" role="tabpanel" aria-labelledby="description-tab">
                        <p className="text-sm w-full xs:text-base md:text-lg text-gray-500 dark:text-slate-200 px-4" dangerouslySetInnerHTML={sanitizedData()}></p>
                    </div>}
                            ...
                            <tbody>
                                {chars.map((char) => (
                                    <tr key={char.id} className="text-left w-full border text-sm">
                                        <th className="font-normal sm:w-52 w-36 py-2 px-4 bg-slate-100 dark:bg-slate-700">{char.name}</th>
                                        <th className="text-slate-600 py-2 px-4 font-normal dark:bg-slate-600 dark:text-slate-200">{char.value}</th>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
