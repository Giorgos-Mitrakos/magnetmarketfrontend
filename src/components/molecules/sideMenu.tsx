    ...
    return (
        <div className='bg-white dark:bg-slate-800 rounded-lg p-5 shadow-sm border border-gray-100 dark:border-slate-700'>
            ...
            <ul className='mt-5 space-y-4'>
                {sideMenu.categories.map(sub => (
                    <li key={sub.slug}>
                        <Link
                            href={`/category/${category1}/${sub.slug}`}
                            className={`block py-2 px-3 rounded-lg text-base font-medium transition-colors ${sub.slug === category2
                        ...
                        {sub.slug === category2 && (
                            <ul className='mt-2 ml-5 pl-4 border-l border-gray-200 dark:border-slate-600 space-y-3'>
                                {sub.categories.map(sub2 => (
                                    <li key={sub2.slug}>
                                        <Link
                                            href={`/category/${category1}/${sub.slug}/${sub2.slug}`}
                                            className={`block py-2 px-3 rounded-lg text-sm transition-colors ${sub2.slug === category3
